defmodule FitconnexWeb.GymOperator.ContestsLive do
  ...
  def handle_event("save_contest", %{"contest" => params}, socket) do
    ...
    case Fitconnex.Gym.create_contest(create_params, actor: actor) do
      {:ok, _contest} ->
        contests = load_contests(gym.id, actor)
  ...
  def handle_event("update_contest", %{"contest" => params}, socket) do
    ...
    if contest do
      ...
      case Fitconnex.Gym.update_contest(contest, update_params, actor: actor) do
        {:ok, _updated} ->
          contests = load_contests(gym.id, actor)
  ...
  def handle_event("delete_contest", %{"id" => id}, socket) do
    ...
    if contest do
      case Fitconnex.Gym.destroy_contest(contest, actor: actor) do
        :ok ->
          contests = load_contests(gym.id, actor)
  ...
  defp load_contests(gym_id, actor) do
    case Fitconnex.Gym.list_contests_by_gym(gym_id, actor: actor) do
