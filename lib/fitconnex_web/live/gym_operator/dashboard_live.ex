      ...
      {:ok, gyms} ->
        case gyms do
          [gym | _] ->
            ...
            plans =
              case Fitconnex.Billing.list_plans_by_gym(gym.id, actor: actor) do
                {:ok, result} -> result
                {:error, _} -> []
              end
                        ...
                        <%= for plan <- Enum.sort_by(type_plans, & &1.price_in_paise) do %>
                          <div class="card bg-base-100 border border-base-300 shadow-sm">
                            <div class="card-body p-4 text-center">
                              <h5 class="font-semibold">{Fitconnex.Billing.PricingHelpers.duration_label(plan.duration)}</h5>
                              <p class="text-2xl font-bold text-primary">
                                Rs{Fitconnex.Billing.PricingHelpers.format_price(plan.price_in_paise)}
                              </p>
                              <% months = Fitconnex.Billing.PricingHelpers.duration_months(plan.duration) %>
                              <%= if months && months > 1 do %>
                                <p class="text-sm text-base-content/60">
                                  Rs{Fitconnex.Billing.PricingHelpers.format_price(Fitconnex.Billing.PricingHelpers.per_month_price(plan.price_in_paise, plan.duration))}/mo
                                </p>
                              <% end %>
                            </div>
                          </div>
