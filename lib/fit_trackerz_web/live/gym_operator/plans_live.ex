defmodule FitTrackerzWeb.GymOperator.PlansLive do
      ...
      {:ok, plan_params_list} ->
        ...
        results =
          Enum.map(plan_params_list, fn params ->
            FitTrackerz.Billing.create_plan(Map.put(params, :gym_id, gym.id), actor: actor)
          end)
  ...
  def handle_event("update_plan", %{"plan" => params}, socket) do
    ...
    if plan do
      ...
      case FitTrackerz.Billing.update_plan(plan, update_params, actor: actor) do
        {:ok, _updated} ->
          plans = load_plans(gym.id, actor)
  ...
  def handle_event("delete_plan", %{"id" => id}, socket) do
    ...
    if plan do
      case FitTrackerz.Billing.destroy_plan(plan, actor: actor) do
        :ok ->
          plans = load_plans(gym.id, actor)
