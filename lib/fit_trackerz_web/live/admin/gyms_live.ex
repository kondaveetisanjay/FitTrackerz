    ...
    <Layouts.app flash={@flash} current_user={@current_user}>
      <div class="space-y-8">
        ...
        <%= if Enum.empty?(@gyms) do %>
          <div class="ft-card p-6" id="gyms-empty-state">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6" id="gyms-grid">
            ...
            >
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="text-center p-2 rounded-lg bg-base-200/30 rounded-xl" id={"gym-location-#{gym.id}"}>
                  <p class="text-lg font-black">{count_loaded(gym.branches)}</p>
                  <p class="text-xs text-base-content/40">Location</p>
                </div>
                <div class="text-center p-2 rounded-lg bg-base-200/30 rounded-xl" id={"gym-members-#{gym.id}"}>
              ...
              <%!-- Actions --%>
              <div class="flex flex-wrap gap-2 pt-2 border-t border-base-200/50">
                <%= if gym.status == :pending_verification do %>
                  <button
                    phx-click="verify_gym"
                    phx-value-id={gym.id}
                    class="btn btn-success btn-xs gap-1 font-medium press-scale"
                ...
                <%= if gym.status != :suspended do %>
                  <button
                    ...
                    phx-value-id={gym.id}
                    class="btn btn-error btn-xs btn-ghost gap-1 font-medium press-scale"
